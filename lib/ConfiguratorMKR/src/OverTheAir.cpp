...
/**
 * @brief Updates the firmware over the air.
 * 
 * This function updates the firmware of the device over the air using HTTP.
 * It takes the server address, port, endpoint, and a flag indicating whether the connection should be secure or not.
 * The function removes automatic update and sets the client to be insecure.
 * It then performs the firmware update using the specified parameters.
 * 
 * @param server The server address to connect to.
 * @param port The port number to use for the connection.
 * @param endpoint The endpoint on the server to send the update request to.
 * @param secure Flag indicating whether the connection should be secure (HTTPS) or not (HTTP).
...
bool OverTheAir::update(String server, int port, String endpoint, bool secure)
{
    ...
    //     client.setCACert(root_ca); // set ssl certificate
    client.setInsecure(); // remove ssl certificate

    DEBUG_PRINTLN(F("Update start now!"));
